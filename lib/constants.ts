...
/** عدد آيات كل سورة (1–114) — لبناء تشغيل المصحف المعلم آية آية */
...
/** إرجاع [أول آية عالمية، آخر آية عالمية] لسورة معينة (1–114) */
export function getSurahAyahRange(surahId: number): [number, number] {
  let start = 1;
  for (let s = 1; s < surahId; s++) {
    start += SURAH_AYAH_COUNTS[s] ?? 0;
  }
  const count = SURAH_AYAH_COUNTS[surahId] ?? 0;
